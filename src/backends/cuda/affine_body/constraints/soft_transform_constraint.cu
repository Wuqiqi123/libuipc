...
namespace
{
        ...
        int                                         n)
    {
        ...
        else
        {
            ...
            Vector12 q_aim  = lerp(q_prev, (external_aims ? transform_to_q(external_aims[i]) : aim_transforms(I)), substep_ratio);
            Vector12 dq     = q - q_aim;
...
class SoftTransformConstraint final : public AffineBodyConstraint
{
  ...
  public:
    ...
    const Matrix4x4* external_targets() const
    {
        return external_aim_transforms ? reinterpret_cast<const Matrix4x4*>(external_aim_transforms.handle())
            + external_aim_transforms.offset() : nullptr;
    }
